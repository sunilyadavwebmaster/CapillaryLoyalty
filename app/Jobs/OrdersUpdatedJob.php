...
class OrdersUpdatedJob implements ShouldQueue
{
    ...
    {
        \Log::info("Called Order Updated webhook construct");
        ...
        try {
            ...
            if($config){
                if($config->cancel_transaction == 1) {
                    ...
                    \Log::info('order update -- '. json_encode($this->data));
                    if(array_key_exists('customer', $data)) {
                        ...
                        if(count($data['refunds']) > 0){
                            foreach($data['refunds'][0]['refund_line_items'] as $cancelItem){
                                foreach ($data['line_items'] as $lineitem) {
                                    if($cancelItem['line_item_id'] == $lineitem['id']) {
                                        $item = array(
                            ...
                            $client = new Client(commonFunction::getClientParams($auth));
                            \Log::info("Order Return : ".json_encode($params));
                            try{
