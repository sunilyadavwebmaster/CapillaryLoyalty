...
class CustomersUpdateJob implements ShouldQueue
{
    ...
    {
        \Log::info("Called customer update webhook construct");
        ...
        try {
            ...
            if($config){
                if($config->updation == 1){
                    ...
                    $url = config('global.api-version').'/'.config('global.update-customer');
                    $registerurl = config('global.api-version').'/'.config('global.add-customer');
                    $params = [
                        "root" => [
                            "customer" => [
                            ...
                            if($attribute['status'] == 1){
                                ...
                                else if(strpos($shopifyAttribute,".")!==false)
                                {
                                    ...
                                    for($i=0;$i<count($mapFieldArray);$i++){
                                        \Log::info('fields map : '.trim($mapFieldArray[$i]));
                                ...
                                if($attribute['field_type'] == 'extended'){
                                    \Log::info('Type : '.$attribute['field_type']);
                                    $field = array(
                    ...
                    try{
                        ...
                        {
                            $result = $client->request('POST', $registerurl, ['json' => $params]);
                        }
                    } catch (ClientException $ex) {
                        \Log::info(Psr7\Message::toString($ex->getResponse()));
                        try{
                            \Log::info(json_encode($params));
                            $result = $client->request('POST', $registerurl, ['json' => $params]);
                        } catch (ClientException $ex1) {
                            \Log::info(Psr7\Message::toString($ex1->getResponse()));
                        } catch(Exception $ex){
                        ...
                        try{
                            ...
                            \Log::info(Psr7\Message::toString($ex2->getResponse()));
                        }catch(Exception $ex){
                            \Log::info($ex->getMessage());
                        }
                    }
                    ...
                    \Log::info('Customer Updation is disabled');
                }
            }else{
